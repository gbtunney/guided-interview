...
This document defines the conversational, AI-guided interview process used to produce field or object specs that conform to the Attribute Spec cheat sheet.

  

It is not a rigid CLI, schema, or form. It is a human-first interview with suggested options and defaults.
...
Advanced topics (nullable, UI flags, platform mapping) are skipped unless explicitly requested.
...
The interview does not enforce a fixed order. Instead, it adapts based on what the user is most confident about.
...
After the entry point, the AI resolves the remaining attributes in a flexible order:
